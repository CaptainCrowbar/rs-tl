...
void test_rs_format_logging() {
    ...
    TRY(log.reset());
    TRY(RS_LOGX("Hello", "world"));
    TRY(RS_LOGX(123, 456, 789));
...
void test_rs_format_logging_output() {
    ...
    for (int i = 1; i <= n_threads; ++i) {
        threads.emplace_back([i] {
            RS_LOGX("Begin thread " + std::to_string(i));
            std::this_thread::sleep_for(100ms);
            RS_LOGX("End thread " + std::to_string(i));
        });
    }
